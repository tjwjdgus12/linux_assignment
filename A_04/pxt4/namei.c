...
int __pxt4_unlink(handle_t *handle, struct inode *dir, const struct qstr *d_name,
		  struct inode *inode)
{
	int retval = -ENOENT;
	struct buffer_head *bh;
...
static int pxt4_unlink(struct inode *dir, struct dentry *dentry)
{
	...
	retval = __pxt4_unlink(handle, dir, &dentry->d_name, d_inode(dentry));
	if (!retval)
		pxt4_fc_track_unlink(handle, dentry);
#ifdef CONFIG_UNICODE
	/* VFS negative dentries are incompatible with Encoding and
		       ...
		       unsigned int flags)
{
	...
	if (S_ISDIR(old.inode->i_mode)) {
		...
		if (whiteout)
			__pxt4_fc_track_create(handle, whiteout, old.dentry);
	}
