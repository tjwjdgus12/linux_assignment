...
#define PXT4_MOUNT_PREFETCH_BLOCK_BITMAPS 0x4000000
...
enum {
	...
	PXT4_MF_FS_ABORTED,	/* Fatal error detected */
	PXT4_MF_FC_INELIGIBLE,	/* Fast commit ineligible */
	PXT4_MF_FC_COMMITTING	/* File system underoing a fast
				 * commit.
				 */
};
...
extern struct buffer_head *pxt4_sb_bread_unmovable(struct super_block *sb,
						   sector_t block);
extern void pxt4_read_bh_nowait(struct buffer_head *bh, int op_flags,
				bh_end_io_t *end_io);
extern int pxt4_read_bh(struct buffer_head *bh, int op_flags,
			bh_end_io_t *end_io);
extern int pxt4_read_bh_lock(struct buffer_head *bh, int op_flags, bool wait);
...
extern __printf(6, 7)
void __pxt4_error(struct super_block *, const char *, unsigned int, int, __u64,
		  const char *, ...);
extern __printf(6, 7)
...
extern void __pxt4_std_error(struct super_block *, const char *,
			     unsigned int, int);
extern __printf(5, 6)
void __pxt4_abort(struct super_block *, const char *, unsigned int, int,
		  const char *, ...);
extern __printf(4, 5)
...
#define pxt4_error(sb, fmt, ...)					\
	__pxt4_error((sb), __func__, __LINE__, 0, 0, (fmt), ##__VA_ARGS__)
#define pxt4_error_err(sb, err, fmt, ...)				\
	__pxt4_error((sb), __func__, __LINE__, (err), 0, (fmt), ##__VA_ARGS__)
#define pxt4_abort(sb, err, fmt, ...)					\
	__pxt4_abort((sb), __func__, __LINE__, (err), (fmt), ##__VA_ARGS__)
...
do {									\
	no_printk(fmt, ##__VA_ARGS__);					\
	__pxt4_error(sb, "", 0, 0, 0, " ");				\
} while (0)
#define pxt4_error_err(sb, err, fmt, ...)				\
do {									\
	no_printk(fmt, ##__VA_ARGS__);					\
	__pxt4_error(sb, "", 0, err, 0, " ");				\
} while (0)
#define pxt4_abort(sb, err, fmt, ...)					\
do {									\
	no_printk(fmt, ##__VA_ARGS__);					\
	__pxt4_abort(sb, "", 0, err, " ");				\
} while (0)
#define pxt4_warning(sb, fmt, ...)					\
do {									\
	no_printk(fmt, ##__VA_ARGS__);					\
...
extern int __pxt4_unlink(handle_t *handle, struct inode *dir, const struct qstr *d_name,
			 struct inode *inode);
