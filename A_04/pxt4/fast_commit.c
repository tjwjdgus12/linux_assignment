 ...
 *
 * - pxt4_fc_mark_ineligible(): This makes next fast commit operation to fall
 *   back to full commit. This is useful in case of transient errors.
 *
 * - pxt4_fc_start_ineligible() and pxt4_fc_stop_ineligible() - This makes all
 *   the fast commits happening between pxt4_fc_start_ineligible() and
 *   pxt4_fc_stop_ineligible() and one fast commit after the call to
 *   pxt4_fc_stop_ineligible() to fall back to full commits. It is important to
 *   make one more fast commit to fall back to full commit after stop call so
 *   that it guaranteed that the fast commit ineligible operation contained
 *   within pxt4_fc_start_ineligible() and pxt4_fc_stop_ineligible() is
 ...
 *    eligible update must be protected within pxt4_fc_start_update() and
 *    pxt4_fc_stop_update(). These routines are called at much higher
 *    routines. This can be made more fine grained by combining with
 *    pxt4_journal_start().
 *
 * 2) Same above for pxt4_fc_start_ineligible() and pxt4_fc_stop_ineligible()
 *
...
void pxt4_fc_start_update(struct inode *inode)
{
	...
	if (!test_opt2(inode->i_sb, JOURNAL_FAST_COMMIT) ||
	    (PXT4_SB(inode->i_sb)->s_mount_state & PXT4_FC_REPLAY))
		return;
 ...
 * operation would result in a full jbd3 commit.
 */
void pxt4_fc_mark_ineligible(struct super_block *sb, int reason)
{
 ...
 */
void pxt4_fc_start_ineligible(struct super_block *sb, int reason)
{
	struct pxt4_sb_info *sbi = PXT4_SB(sb);
...
/*
 * Stop a fast commit ineligible update. We set PXT4_MF_FC_INELIGIBLE flag here
 * to ensure that after stopping the ineligible update, at least one full
 * commit takes place.
 */
void pxt4_fc_stop_ineligible(struct super_block *sb)
{
	if (!test_opt2(sb, JOURNAL_FAST_COMMIT) ||
	    (PXT4_SB(sb)->s_mount_state & PXT4_FC_REPLAY))
		return;
	...
	pxt4_set_mount_flag(sb, PXT4_MF_FC_INELIGIBLE);
	atomic_dec(&PXT4_SB(sb)->s_fc_ineligible_updates);
}

static inline int pxt4_fc_is_ineligible(struct super_block *sb)
{
	return (pxt4_test_mount_flag(sb, PXT4_MF_FC_INELIGIBLE) ||
		atomic_read(&PXT4_SB(sb)->s_fc_ineligible_updates));
}
	...
	void *args, int enqueue)
{
	...
	if (pxt4_fc_is_ineligible(inode->i_sb))
		return -EINVAL;
	...
	if (list_empty(&PXT4_I(inode)->i_fc_list))
		list_add_tail(&PXT4_I(inode)->i_fc_list,
				(pxt4_test_mount_flag(inode->i_sb, PXT4_MF_FC_COMMITTING)) ?
...
static int __track_dentry_update(struct inode *inode, void *arg, bool update)
{
	...
	if (!node) {
		pxt4_fc_mark_ineligible(inode->i_sb, PXT4_FC_REASON_NOMEM);
		mutex_lock(&ei->i_fc_lock);
		return -ENOMEM;
	}
	...
	spin_lock(&sbi->s_fc_lock);
	if (pxt4_test_mount_flag(inode->i_sb, PXT4_MF_FC_COMMITTING))
		...
		struct inode *inode, struct dentry *dentry)
{
	...
	ret = pxt4_fc_track_template(handle, inode, __track_dentry_update,
					(void *)&args, 0);
	trace_pxt4_fc_track_unlink(inode, dentry, ret);
}

void pxt4_fc_track_unlink(handle_t *handle, struct dentry *dentry)
{
	__pxt4_fc_track_unlink(handle, d_inode(dentry), dentry);
}
	...
	struct inode *inode, struct dentry *dentry)
{
	...
	ret = pxt4_fc_track_template(handle, inode, __track_dentry_update,
					(void *)&args, 0);
	trace_pxt4_fc_track_link(inode, dentry, ret);
}

void pxt4_fc_track_link(handle_t *handle, struct dentry *dentry)
{
	__pxt4_fc_track_link(handle, d_inode(dentry), dentry);
}
			  ...
			  struct dentry *dentry)
{
	...
	ret = pxt4_fc_track_template(handle, inode, __track_dentry_update,
					(void *)&args, 0);
	trace_pxt4_fc_track_create(inode, dentry, ret);
}

void pxt4_fc_track_create(handle_t *handle, struct dentry *dentry)
{
	__pxt4_fc_track_create(handle, d_inode(dentry), dentry);
}
...
void pxt4_fc_track_inode(handle_t *handle, struct inode *inode)
{
	...
	trace_pxt4_fc_track_inode(inode, ret);
}
			 ...
			 pxt4_lblk_t end)
{
	...
	trace_pxt4_fc_track_range(inode, start, end, ret);
}

static void pxt4_fc_submit_bh(struct super_block *sb)
{
...
static void *pxt4_fc_memzero(struct super_block *sb, void *dst, int len,
				u32 *crc)
{
	void *ret;
	...
	if (crc)
		*crc = pxt4_chksum(PXT4_SB(sb), *crc, dst, len);
	return ret;
}
...
static u8 *pxt4_fc_reserve_space(struct super_block *sb, int len, u32 *crc)
{
	 ...
	 * padding.
	 */
	if (len + sizeof(struct pxt4_fc_tl) > bsize)
		return NULL;

	if (bsize - off - 1 > len + sizeof(struct pxt4_fc_tl)) {
		/*
	...
	/* Need to add PAD tag */
	tl = (struct pxt4_fc_tl *)(sbi->s_fc_bh->b_data + off);
	tl->fc_tag = cpu_to_le16(PXT4_FC_TAG_PAD);
	pad_len = bsize - off - 1 - sizeof(struct pxt4_fc_tl);
	tl->fc_len = cpu_to_le16(pad_len);
	if (crc)
		*crc = pxt4_chksum(sbi, *crc, tl, sizeof(*tl));
	if (pad_len > 0)
		pxt4_fc_memzero(sb, tl + 1, pad_len, crc);
	pxt4_fc_submit_bh(sb);
...
static void *pxt4_fc_memcpy(struct super_block *sb, void *dst, const void *src,
				int len, u32 *crc)
{
	if (crc)
		*crc = pxt4_chksum(PXT4_SB(sb), *crc, src, len);
...
static int pxt4_fc_write_tail(struct super_block *sb, u32 crc)
{
	 ...
	 */
	dst = pxt4_fc_reserve_space(sb, sizeof(tl) + sizeof(tail), &crc);
	if (!dst)
		return -ENOSPC;
	...
	tl.fc_len = cpu_to_le16(bsize - off - 1 + sizeof(struct pxt4_fc_tail));
	sbi->s_fc_bytes = round_up(sbi->s_fc_bytes, bsize);

	pxt4_fc_memcpy(sb, dst, &tl, sizeof(tl), &crc);
	dst += sizeof(tl);
	tail.fc_tid = cpu_to_le32(sbi->s_journal->j_running_transaction->t_tid);
	pxt4_fc_memcpy(sb, dst, &tail.fc_tid, sizeof(tail.fc_tid), &crc);
	dst += sizeof(tail.fc_tid);
	tail.fc_crc = cpu_to_le32(crc);
	pxt4_fc_memcpy(sb, dst, &tail.fc_crc, sizeof(tail.fc_crc), NULL);
			   ...
			   u32 *crc)
{
	...
	dst = pxt4_fc_reserve_space(sb, sizeof(tl) + len, crc);
	if (!dst)
		return false;
	...
	pxt4_fc_memcpy(sb, dst, &tl, sizeof(tl), crc);
	pxt4_fc_memcpy(sb, dst + sizeof(tl), val, len, crc);

	return true;
}

/* Same as above, but adds dentry tlv. */
static  bool pxt4_fc_add_dentry_tlv(struct super_block *sb, u16 tag,
					...
					u32 *crc)
{
	...
	struct pxt4_fc_tl tl;
	u8 *dst = pxt4_fc_reserve_space(sb, sizeof(tl) + sizeof(fcd) + dlen,
					crc);
	...
	pxt4_fc_memcpy(sb, dst, &tl, sizeof(tl), crc);
	dst += sizeof(tl);
	pxt4_fc_memcpy(sb, dst, &fcd, sizeof(fcd), crc);
	dst += sizeof(fcd);
	pxt4_fc_memcpy(sb, dst, dname, dlen, crc);
	dst += dlen;
...
static int pxt4_fc_write_inode(struct inode *inode, u32 *crc)
{
	...
	if (!pxt4_fc_memcpy(inode->i_sb, dst, &tl, sizeof(tl), crc))
		return -ECANCELED;
	dst += sizeof(tl);
	if (!pxt4_fc_memcpy(inode->i_sb, dst, &fc_inode, sizeof(fc_inode), crc))
		return -ECANCELED;
	dst += sizeof(fc_inode);
	if (!pxt4_fc_memcpy(inode->i_sb, dst, (u8 *)pxt4_raw_inode(&iloc),
					inode_len, crc))
		return -ECANCELED;
...
static int pxt4_fc_submit_inode_data_all(journal_t *journal)
{
	...
	spin_lock(&sbi->s_fc_lock);
	pxt4_set_mount_flag(sb, PXT4_MF_FC_COMMITTING);
	list_for_each(pos, &sbi->s_fc_q[FC_Q_MAIN]) {
		ei = list_entry(pos, struct pxt4_inode_info, i_fc_list);
		...
		spin_unlock(&sbi->s_fc_lock);
		ret = jbd3_submit_inode_data(ei->jinode);
		if (ret)
			return ret;
		spin_lock(&sbi->s_fc_lock);
	}
...
__releases(&sbi->s_fc_lock)
{
	...
	list_for_each_safe(fcd_pos, fcd_n, &sbi->s_fc_dentry_q[FC_Q_MAIN]) {
		fc_dentry = list_entry(fcd_pos, struct pxt4_fc_dentry_update,
					fcd_list);
		if (fc_dentry->fcd_op != PXT4_FC_TAG_CREAT) {
			spin_unlock(&sbi->s_fc_lock);
			if (!pxt4_fc_add_dentry_tlv(
				sb, fc_dentry->fcd_op,
...
static int pxt4_fc_perform_commit(journal_t *journal)
{
	...
	list_for_each(pos, &sbi->s_fc_q[FC_Q_MAIN]) {
		iter = list_entry(pos, struct pxt4_inode_info, i_fc_list);
...
int pxt4_fc_commit(journal_t *journal, tid_t commit_tid)
{
	...
	int reason = PXT4_FC_REASON_OK, fc_bufs_before = 0;
	ktime_t start_time, commit_time;

	trace_pxt4_fc_commit_start(sb);

	start_time = ktime_get();

	if (!test_opt2(sb, JOURNAL_FAST_COMMIT) ||
		(pxt4_fc_is_ineligible(sb))) {
		reason = PXT4_FC_REASON_INELIGIBLE;
		goto out;
	}
...
restart_fc:
	...
	if (ret == -EALREADY) {
		...
		if (atomic_read(&sbi->s_fc_subtid) <= subtid &&
			...
			goto restart_fc;
		reason = PXT4_FC_REASON_ALREADY_COMMITTED;
		goto out;
	} else if (ret) {
		sbi->s_fc_stats.fc_ineligible_reason_count[PXT4_FC_COMMIT_FAILED]++;
		reason = PXT4_FC_REASON_FC_START_FAILED;
		goto out;
	}
	...
	if (ret < 0) {
		sbi->s_fc_stats.fc_ineligible_reason_count[PXT4_FC_COMMIT_FAILED]++;
		reason = PXT4_FC_REASON_FC_FAILED;
		goto out;
	}
	...
	jbd3_fc_end_commit(journal);
out:
	/* Has any ineligible update happened since we started? */
	if (reason == PXT4_FC_REASON_OK && pxt4_fc_is_ineligible(sb)) {
		sbi->s_fc_stats.fc_ineligible_reason_count[PXT4_FC_COMMIT_FAILED]++;
		reason = PXT4_FC_REASON_INELIGIBLE;
	}

	spin_lock(&sbi->s_fc_lock);
	if (reason != PXT4_FC_REASON_OK &&
		reason != PXT4_FC_REASON_ALREADY_COMMITTED) {
	...
	spin_unlock(&sbi->s_fc_lock);
	nblks = (reason == PXT4_FC_REASON_OK) ? nblks : 0;
	trace_pxt4_fc_commit_stop(sb, nblks, reason);
	...
	jbd_debug(1,
		...
		nblks, reason, subtid);
	if (reason == PXT4_FC_REASON_FC_FAILED)
		return jbd3_fc_end_commit_fallback(journal);
	if (reason == PXT4_FC_REASON_FC_START_FAILED ||
		reason == PXT4_FC_REASON_INELIGIBLE)
 ...
 */
static void pxt4_fc_cleanup(journal_t *journal, int full)
{
	...
	pxt4_clear_mount_flag(sb, PXT4_MF_FC_COMMITTING);
			      ...
			      struct  pxt4_fc_tl *tl, u8 *val)
{
	...
	darg->dname_len = le16_to_cpu(tl->fc_len) -
		sizeof(struct pxt4_fc_dentry_info);
}

/* Unlink replay function */
static int pxt4_fc_replay_unlink(struct super_block *sb, struct pxt4_fc_tl *tl,
				 u8 *val)
{
	...
	ret = __pxt4_unlink(NULL, old_parent, &entry, inode);
...
static int pxt4_fc_replay_link(struct super_block *sb, struct pxt4_fc_tl *tl,
			       u8 *val)
{
	struct inode *inode;
	struct dentry_info_args darg;
 ...
 */
static int pxt4_fc_replay_inode(struct super_block *sb, struct pxt4_fc_tl *tl,
				u8 *val)
{
	struct pxt4_fc_inode fc_inode;
	...
	pxt4_fc_record_modified_inode(sb, ino);
	...
	inode_len = le16_to_cpu(tl->fc_len) - sizeof(struct pxt4_fc_inode);
	...
	memcpy(&raw_inode->i_generation, &raw_fc_inode->i_generation,
		inode_len - offsetof(struct pxt4_inode, i_generation));
 ...
 */
static int pxt4_fc_replay_create(struct super_block *sb, struct pxt4_fc_tl *tl,
				 u8 *val)
{
	int ret = 0;
	struct inode *inode = NULL;
 ...
 */
static int pxt4_fc_record_regions(struct super_block *sb, int ino,
		pxt4_lblk_t lblk, pxt4_fsblk_t pblk, int len)
{
	struct pxt4_fc_replay_state *state;
...
static int
pxt4_fc_replay_del_range(struct super_block *sb, struct pxt4_fc_tl *tl,
			 u8 *val)
{
	struct inode *inode;
	struct pxt4_fc_del_range lrange;
	...
	ret = pxt4_punch_hole(inode,
		le32_to_cpu(lrange.fc_lblk) << sb->s_blocksize_bits,
		le32_to_cpu(lrange.fc_len) <<  sb->s_blocksize_bits);
	if (ret)
		jbd_debug(1, "pxt4_punch_hole returned %d", ret);
				...
				enum passtype pass, int off, tid_t expected_tid)
{
	...
	for (cur = start; cur < end; cur = cur + sizeof(tl) + le16_to_cpu(tl.fc_len)) {
		...
		case PXT4_FC_TAG_PAD:
			...
			break;
		case PXT4_FC_TAG_TAIL:
			trace_pxt4_fc_replay(sb, PXT4_FC_TAG_TAIL, 0,
					     le16_to_cpu(tl.fc_len), 0);
			memcpy(&tail, val, sizeof(tail));
		...
		case PXT4_FC_TAG_HEAD:
			break;
		default:
			trace_pxt4_fc_replay(sb, le16_to_cpu(tl.fc_tag), 0,
					     le16_to_cpu(tl.fc_len), 0);
			ret = -ECANCELED;
			break;
		}
