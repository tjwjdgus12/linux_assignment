...
static inline void __pxt4_read_bh(struct buffer_head *bh, int op_flags,
				  bh_end_io_t *end_io)
{
	/*
...
void pxt4_read_bh_nowait(struct buffer_head *bh, int op_flags,
			 bh_end_io_t *end_io)
{
...
int pxt4_read_bh(struct buffer_head *bh, int op_flags, bh_end_io_t *end_io)
{
	BUG_ON(!buffer_locked(bh));
...
int pxt4_read_bh_lock(struct buffer_head *bh, int op_flags, bool wait)
{
...
void pxt4_sb_breadahead_unmovable(struct super_block *sb, sector_t block)
{
	...
	if (likely(bh)) {
		pxt4_read_bh_lock(bh, REQ_RAHEAD, false);
		brelse(bh);
	}
}
			      ...
			      const char *func, unsigned int line)
{
	...
	case ENOMEM:
		...
		break;
	case EFSBADCRC:
		err = PXT4_ERR_EFSBADCRC;
		break;
	case 0:
	case EFSCORRUPTED:
		err = PXT4_ERR_EFSCORRUPTED;
		break;
	case ENOSPC:
		err = PXT4_ERR_ENOSPC;
		break;
	...
	case ERANGE:
		...
		break;
	case EOVERFLOW:
		err = PXT4_ERR_EOVERFLOW;
		break;
	case EBUSY:
		err = PXT4_ERR_EBUSY;
		break;
	case ENOTDIR:
		err = PXT4_ERR_ENOTDIR;
		break;
	case ENOTEMPTY:
		err = PXT4_ERR_ENOTEMPTY;
		break;
	case ESHUTDOWN:
		err = PXT4_ERR_ESHUTDOWN;
		break;
	case EFAULT:
		err = PXT4_ERR_EFAULT;
		break;
	default:
		err = PXT4_ERR_UNKNOWN;
	}
 ...
 */
static int pxt4_journalled_writepage_callback(struct page *page,
...
static int pxt4_journal_submit_inode_data_buffers(struct jbd3_inode *jinode)
{
	...
	if (pxt4_should_journal_data(jinode->i_vfs_inode))
		...
		ret = jbd3_journal_submit_inode_data_buffers(jinode);
...
static void pxt4_handle_error(struct super_block *sb)
{
	...
	pxt4_set_mount_flag(sb, PXT4_MF_FS_ABORTED);
	if (journal)
...
static const struct mount_opts {
	...
	{Opt_dax, PXT4_MOUNT_DAX_ALWAYS, MOPT_SET | MOPT_SKIP},
	{Opt_dax_always, PXT4_MOUNT_DAX_ALWAYS,
		MOPT_PXT4_ONLY | MOPT_SET | MOPT_SKIP},
	{Opt_dax_inode, PXT4_MOUNT2_DAX_INODE,
		MOPT_PXT4_ONLY | MOPT_SET | MOPT_SKIP},
	{Opt_dax_never, PXT4_MOUNT2_DAX_NEVER,
		MOPT_PXT4_ONLY | MOPT_SET | MOPT_SKIP},
	...
	{Opt_prefetch_block_bitmaps, PXT4_MOUNT_PREFETCH_BLOCK_BITMAPS,
	 MOPT_SET},
#ifdef CONFIG_PXT4_DEBUG
...
static int pxt4_set_test_dummy_encryption(struct super_block *sb,
					  const char *opt,
			    ...
			    unsigned int *journal_ioprio, int is_remount)
{
...
#else
		pxt4_msg(sb, KERN_ERR, "inline encryption not supported");
#endif
		return 1;
	}
	...
	if (m->flags & MOPT_NOSUPPORT) {
		...
		*journal_ioprio =
		...
		return pxt4_set_test_dummy_encryption(sb, opt, &args[0],
						      is_remount);
			 ...
			 int is_remount)
{
	...
	if (test_opt(sb, DIOREAD_NOLOCK)) {
		...
		if (blocksize < PAGE_SIZE)
			pxt4_msg(sb, KERN_WARNING, "Warning: mounting with an "
...
static int pxt4_run_li_request(struct pxt4_li_request *elr)
{
	...
	if (elr->lr_mode == PXT4_LI_MODE_PREFETCH_BBITMAP) {
		elr->lr_next_group = pxt4_mb_prefetch(sb, group,
				PXT4_SB(sb)->s_mb_prefetch, &prefetch_ios);
		if (prefetch_ios)
			pxt4_mb_prefetch_fini(sb, elr->lr_next_group,
					      prefetch_ios);
		trace_pxt4_prefetch_bitmaps(sb, group, elr->lr_next_group,
					    prefetch_ios);
...
static int pxt4_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	if (test_opt(sb, DATA_FLAGS) == PXT4_MOUNT_JOURNAL_DATA) {
		printk_once(KERN_WARNING "PXT4-fs: Warning: mounting with data=journal disables delayed allocation, dioread_nolock, O_DIRECT and fast_commit support!\n");
...
#endif
	fscrypt_free_dummy_policy(&sbi->s_dummy_enc_policy);
	/* pxt4_blkdev_remove() calls kill_bdev(), release bh before it. */
	brelse(bh);
	pxt4_blkdev_remove(sbi);
out_fail:
	sb->s_fs_info = NULL;
				       ...
				       dev_t j_dev)
{
	...
	journal->j_private = sb;
	if (pxt4_read_bh_lock(journal->j_sb_buffer, REQ_META | REQ_PRIO, true)) {
	...
	PXT4_SB(sb)->s_journal_bdev = bdev;
	pxt4_init_journal_params(sb, journal);
	return journal;
...
static int pxt4_remount(struct super_block *sb, int *flags, char *data)
{
	...
	if (pxt4_test_mount_flag(sb, PXT4_MF_FS_ABORTED))
		pxt4_abort(sb, PXT4_ERR_ESHUTDOWN, "Abort forced by user");
