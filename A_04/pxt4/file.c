				 ...
				 int error, unsigned int flags)
{
	 ...
	 * of on-disk size will happen later in pxt4_dio_write_iter() where
...
restart:
	...
	offset = iocb->ki_pos;
	count = ret;
	if (pxt4_extending_io(inode, offset, count))
		*extend = true;
	/*
	...
	if (*ilock_shared && (!IS_NOSEC(inode) || *extend ||
	     !pxt4_overwrite_io(inode, offset, count))) {
		if (iocb->ki_flags & IOCB_NOWAIT) {
			ret = -EAGAIN;
			goto out;
		}
...
static ssize_t pxt4_dio_write_iter(struct kiocb *iocb, struct iov_iter *from)
{
	 ...
	 */
	if (pxt4_unaligned_io(inode, from, offset)) {
		unaligned_io = true;
		ilock_shared = false;
	}
	/*
